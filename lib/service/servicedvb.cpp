...
RESULT eDVBServicePlay::getPlayPosition(pts_t &pos)
{
	...
	// Case 2: Normal hardware decoder
		 ...
		 *
		 * NOT extended to m_skipmode != 0 (FF(16+) skipmode trickmode): in
		 * that mode the decoder consumes file-cursor-skipped frames whose
		 * pts_video races ahead at the skip rate. Using video PTS for the
		 * UI then makes the time display race and overshoot the file end
		 * → time display garbage + EOF action triggers "jump to end". */
